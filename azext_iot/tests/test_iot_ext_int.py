...
class TestIoTHub(LiveScenarioTest):
    ...
    def _create_entity_names(self, devices=0, edge_devices=0, modules=0, configs=0):
        ...
        if devices:
            ...
            for _ in range(devices):
                device_ids.append(
                    self.create_random_name(prefix=DEVICE_PREFIX, length=32)
                )
        ...
        if edge_devices:
            ...
            for _ in range(edge_devices):
                edge_device_ids.append(
                    self.create_random_name(prefix="test-edge-device-", length=32)
                )
        ...
        if modules:
            ...
            for _ in range(modules):
                module_ids.append(
                    self.create_random_name(prefix="test-module-", length=32)
                )
        ...
        if configs:
            ...
            for _ in range(configs):
                config_ids.append(
                    self.create_random_name(prefix="test-config-", length=32)
                )
        ...
        for i in range(edge_device_count):
            self.cmd(
                "iot hub device-identity create -d {} -n {} -g {} --ee --add-children {} --force".format(
                    edge_device_ids[i], LIVE_HUB, LIVE_RG, device_ids[4]
                ),
                checks=[
            ...
            device_scope_str_pattern = r"^{}{}-".format(
                DEVICE_DEVICESCOPE_PREFIX, edge_device_ids[i]
            )
            self.cmd(
    ...
    def test_hub_apply_device_content(self):
        ...
        content_path_malformed = os.path.join(CWD, "test_config_modules_content_malformed.json")
        ...
        self.cmd(
            "iot hub apply-configuration -d {} -n {} -g {} -k '{}'".format(
                edge_device_ids[0], LIVE_HUB, LIVE_RG, content_path
            ),
            checks=[self.check("length([*])", 3)],
        )

        self.cmd(
            "iot hub apply-configuration -d {} -n {} -g {} --content '{}'".format(
                edge_device_ids[0], LIVE_HUB, LIVE_RG, "{generic_content}"
            ),
            self.check("length([*])", 3),
        )
        ...
        # With connection string
        self.cmd(
            "iot hub apply-configuration -d {} --login {} -k '{}'".format(
                edge_device_ids[0], LIVE_HUB_CS, content_path_v1
            ),
    ...
    def test_hub_monitor_events(self):
        ...
        self.command_execute_assert(
            "iot hub monitor-events -n {} -g {} -d {} --et {} -t 10 -y -p sys anno app".format(
                LIVE_HUB, LIVE_RG, DEVICE_PREFIX + "*", enqueued_time
            ),
            device_ids,
        )
