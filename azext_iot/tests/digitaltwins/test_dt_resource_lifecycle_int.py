...
class TestDTResourceLifecycle(DTLiveScenarioTest):
    def __init__(self, test_case):
        super(TestDTResourceLifecycle, self).__init__(test_case, required_capacity=3)
    ...
    def test_dt_endpoints_routes(self):
        ...
        add_ep_sb_identity_output = self.cmd(
            "dt endpoint create servicebus -n {} --sbg {} --sbn {} --sbt {} --en {} --auth-type IdentityBased".format(
                endpoints_instance_name,
                servicebus_rg,
                servicebus_namespace,
                servicebus_topic,
        ...
        add_ep_output = self.cmd(
            "dt endpoint create eventhub -n {} --ehg {} --ehn {} --eh {} --ehs {} --en {} --auth-type IdentityBased".format(
                endpoints_instance_name,
                eventhub_rg,
                eventhub_namespace,
                eventhub_topic,
...
def assert_common_endpoint_attributes(
    ...
    if dead_letter_endpoint:
        assert endpoint_output["properties"]["deadLetterEndpoint"]
