...
def generate_device_config(
    ...
    metrics_from_file=False,
    content_from_file=False,
    metrics_short_form=False,
    ...
    if content_from_file:
        ...
        if content_type == "modules":
            content_key = (
                "moduleContent" if modules_schema == "1.0" else "modulesContent"
            )
...
class TestConfigCreate:
    ...
    def test_config_create(self, serviceclient, req):
        if req["content_type"] == "device":
            subject.iot_hub_configuration_create(
                ...
                hub_name=mock_target["entity"],
                content=req["content"],
                target_condition=req["targetCondition"],
    ...
    @pytest.mark.parametrize("req", [(generate_device_config(content_type="device"))])
...
class TestConfigShow:
    ...
    def serviceclient(self, mocker, fixture_ghcs, fixture_sas, request):
        ...
        service_client.return_value = build_mock_response(
            ...
            request.param[0],
            generate_device_config(
                scenario="update", content_type=request.param[1], config_id=config_id
            ),
        )
    ...
    def test_config_show(self, serviceclient):
        ...
        assert result == generate_device_config(
            scenario="update", content_type=serviceclient.test_meta, config_id=config_id
        )
        assert json.dumps(result)
...
class TestConfigMetricShow:
    ...
    def serviceclient(self, mocker, fixture_ghcs, fixture_sas, request):
        ...
        service_client.side_effect = [
            build_mock_response(mocker, payload=generate_config_metrics()),
            build_mock_response(
    ...
    def test_config_metric_show(
        self, serviceclient, metric_id, content_type, metric_type
    ):

        if content_type == "modules":
            result = subject.iot_edge_deployment_metric_show(
                fixture_cmd,
                config_id=config_id,
                metric_id=metric_id,
                hub_name=mock_target["entity"],
            ...
            result = subject.iot_hub_configuration_metric_show(
                fixture_cmd,
                config_id=config_id,
                metric_type=mt,
                metric_id=metric_id,
    ...
    def test_config_metric_show_invalid_args(
        ...
        with pytest.raises(CLIError):
            if content_type == "modules":
                subject.iot_edge_deployment_metric_show(
                    fixture_cmd,
                    config_id=config_id,
                    metric_id=metric_id,
                    hub_name=mock_target["entity"],
                ...
                mt = "user" if metric_type == "metrics" else "system"
                subject.iot_hub_configuration_metric_show(
                    fixture_cmd,
                    config_id=config_id,
                    metric_type=mt,
                    metric_id=metric_id,
    ...
    def test_config_metric_show_error(
        self, serviceclient_generic_error, metric_id, content_type, metric_type
    ):
        with pytest.raises(CLIError):
            if content_type == "modules":
                subject.iot_edge_deployment_metric_show(
